# IT Fundamentals Module 04: Resources

## Official Documentation

### APIs

- [REST (MDN Web Docs)](https://developer.mozilla.org/en-US/docs/Glossary/REST)
- [JSON (MDN Web Docs)](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Objects/JSON)
- [HTTP Request Methods (MDN Web Docs)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods)

### Programming Basics (Python)

- [The Python Tutorial (python.org)](https://docs.python.org/3/tutorial/)
- [Python Data Model (python.org)](https://docs.python.org/3/reference/datamodel.html)
- [Built-in Types (python.org)](https://docs.python.org/3/library/stdtypes.html)
- [Defining Functions (python.org)](https://docs.python.org/3/tutorial/controlflow.html#defining-functions)
- [JSON library (python.org)](https://docs.python.org/3/library/json.html)

## AWS Documentation References

- [What Is AWS Lambda?](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html)
- [What Is Amazon S3?](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html)

## Recommended Learning Resources

- [The Python Tutorial (python.org)](https://docs.python.org/3/tutorial/)

---

*AWS Bootcamp: From Novice to Architect*
*Author: Samuel Ogunti*
*License: [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/)*
